...
In the order service API, an in-memory cache dictionary is initialized. Database queries (cache misses) are added to the cache dictionary to serve future orders/{order_id} endpoint calls. If an order is updated, then the order is cleared from the cache dictionary. On the subsequent read, the order is queried from the database to ensure correctness on the specific node. Structured logging is implemented to record cache hits and misses. A delay of 0.1 seconds is added to database queries to simulate network latency in a cloud deployment scenario.
...
7. Execute the following commands to check the database tables after creating the order to confirm if one order is created (one is correct): docker exec -it $(docker ps -qf "name=postgres") psql -U {DB_USER} -d {DB_NAME}
...
19. Execute the following commands to check the database tables after updating the order in step #16 to confirm if one order exists (one is correct):
